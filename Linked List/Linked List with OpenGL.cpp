...
void insert_circle_specific(int n, int x, int y)
{
	...
	temp = first;
	prev = first;
	Circle *newC = new Circle(x, y, size);
	if (temp == NULL)
		cout << "LIST Empty" << endl;
	else{
		while (++count!= n){
		...
		else if (temp->next == NULL)	//If you want to insert a circle at the last node
		{
			temp->next = newC;
			newC->next = NULL;
		}
...
void myMouseFunction( int button, int state, int mouseX, int mouseY ) 
{
	...
	if(state==1 && button==0){
		if(first==NULL){								//if list is empty
			first= new Circle(mouseX,400-mouseY,size);	//Create first circle
			first->next=NULL;							//set the next as null
			current=first;								//set the current as first
			//first->draw_circle();
		}	
		else{
				Circle *temp= new Circle(mouseX,400-mouseY,size);	//At the end create a temporary Circle
...
void myKeyboardFunction( unsigned char key, int mouseX, int mouseY )
{
	...
	else if(key == 'x')		//Delete specific circle from linked list
	{
		...
		int n;
		cout << "Which circle do you want to delete" << endl;
		cin >> n;
	...
	else if (key == 'a')
	{
		...
		cout << "In which node do you want to insert the circle: " << endl;
		cin >> node;
