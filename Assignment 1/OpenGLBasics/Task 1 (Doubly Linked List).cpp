...
For traversion, press left or right keyboard key in open GL window to traverse backward or forward respectively. When once the left or right key is pressed, it will traverse one position forward or backward depending opon the key you press. Then it will ask you to choose the color of the circle you are standing on. The rest is self explanatory. 
*/
...
void insert_circle_specific(int n, int x, int y)
{
	...
	else
	{
		...
		else if (temp->next != NULL)	//If you want to insert a circle in between
		{
			prev->next = newC;
			newC->next = temp;
			temp->prev = newC;
		...
		else if (temp->next == NULL)	//If you want to insert a circle at the second last node
		{
			Circle *temp2 = temp->prev;
			temp->prev = newC;
			newC->prev = temp2;
			temp2->next = newC;
			newC->next = temp;
		...
		while (temp != NULL) {
			...
			cout << " Previous address of Current Node=" << temp->prev << endl;
			cout << " Address of Current Node=" << temp << endl;
			cout << " Next address of Current Node=" << temp->next << endl << endl << endl;
			temp = temp->next;
...
void myMouseFunction(int button, int state, int mouseX, int mouseY)
{
	...
	if (state == 1 && button == 0) {
		if (first == NULL) {								//if list is empty
			first = new Circle(mouseX, 400 - mouseY, 5);	//Create first circle
			first->next = NULL;							//set the next as null
			current = first;								//set the current as first
			current->prev = NULL;
			//first->draw_circle();
		}
		else {
			Circle *temp = new Circle(mouseX, 400 - mouseY, 5);	//At the end create a temporary Circle
			Circle *temp2 = current;
...
void mySpecialKeysFunction(int key, int x, int y)
{
	...
	case GLUT_KEY_LEFT:
		if (node_circle == 0)
		{
			cout << "\nYou have successfully traversed one position to the left";
		...
		else
		{
			...
			if (current->prev != NULL)
			{
				...
				cout << "Failed to traverse as there is no node before this!" << endl;
		}
	...
	case GLUT_KEY_RIGHT:
		...
		else
		{
			...
			if (current->next != NULL)
			{
				...
				cout << "Failed to traverse as there is no node after this!" << endl;
		}
		break;
	case GLUT_KEY_HOME:
		break;
